    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push(
        <TouchableOpacity key={day} style={styles.dayCell} onPress={() => setSelectedDate(day)}>
          <View style={[styles.dayNumber, isTodayDate && styles.todayNumber, isSelected && styles.selectedNumber]}>
            <Text style={[styles.dayText, isTodayDate && styles.todayText, isSelected && styles.selectedText]}>{day}</Text>
          </View>
          {hasEventDate && <View style={styles.eventDot}><Text style={styles.eventDotText}>•</Text></View>}
        </TouchableOpacity>
      );
    }
    return days;
        ...
        <ScrollView style={styles.scrollContent} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.calendarContainer}>
            ...
            <View style={styles.dayHeaders}>
              {dayNames.map(day => (
                <View key={day} style={styles.dayHeader}><Text style={styles.dayHeaderText}>{day}</Text></View>
              ))}
            </View>
          ...
          <View style={styles.eventsSection}>
            {selectedDate ? (
              <>
                <Text style={styles.eventDate}>{formatDateForDisplay(selectedDate)}</Text>
                {getSelectedDateEvents().length > 0 ? (
                  getSelectedDateEvents().map(event => (
                    <TouchableOpacity key={event.id} style={styles.eventItem}>
                      <View style={styles.eventContent}>
                        <Text style={styles.eventTitle}>{event.title}</Text>
                        <Text style={styles.eventDetails}>{event.date} {event.time} @{event.location}</Text>
                      </View>
                      <Text style={styles.eventArrow}>›</Text>
                    </TouchableOpacity>
                  ))
                ) : (
                  <View style={styles.noEventItem}><Text style={styles.noEventText}>이 날짜에는 일정이 없습니다</Text></View>
                )}
              </>
            ) : (
              <>
                <Text style={styles.eventDate}>다가오는 일정</Text>
                {events.filter(e => new Date(e.date) >= today).slice(0, 3).map(event => {
                  ...
                  return (
                    <TouchableOpacity key={event.id} style={styles.eventItem}>
                      <View style={styles.eventContent}>
                        <Text style={styles.eventTitle}>{event.title}</Text>
                        <Text style={styles.eventDetails}>{month}월 {day}일 {event.time} @{event.location}</Text>
                      </View>
                      <Text style={styles.eventArrow}>›</Text>
                    </TouchableOpacity>
                  );
...
const styles = StyleSheet.create({
  ...
  countBadge: { backgroundColor: '#FFF', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 16, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.04, shadowRadius: 4, elevation: 2 },
  countText: { fontSize: 12, fontWeight: '500', color: '#000' },

  scrollContent: { flex: 1 },
  titleSection: { paddingHorizontal: 28, marginBottom: 24 },
  pageTitle: { fontSize: 28, fontWeight: '700', color: '#000', letterSpacing: -0.5 },

  calendarContainer: { backgroundColor: '#FFF', marginHorizontal: 28, borderRadius: 16, padding: 20, marginBottom: 24, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.08, shadowRadius: 12, elevation: 4 },
  ...
  eventDot: { position: 'absolute', bottom: 2, left: '50%', transform: [{ translateX: -15 }] },
  eventDotText: { fontSize: 12, fontWeight: '700', color: '#FF3B30' },

  eventsSection: { paddingHorizontal: 28, paddingBottom: 40 },
  eventDate: { fontSize: 18, fontWeight: '700', color: '#000', marginBottom: 12, marginTop: 20 },
  eventItem: { backgroundColor: '#FFF', borderRadius: 12, padding: 16, flexDirection: 'row', alignItems: 'center', marginBottom: 8, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.04, shadowRadius: 4, elevation: 2 },
  eventContent: { flex: 1 },
  eventTitle: { fontSize: 16, fontWeight: '600', color: '#000', marginBottom: 4 },
  eventDetails: { fontSize: 12, fontWeight: '400', color: '#8E8E93' },
  noEventItem: { backgroundColor: '#F8F9FA', borderRadius: 12, padding: 16, alignItems: 'center' },
  ...
  bottomNav: { flexDirection: 'row', justifyContent: 'space-around', alignItems: 'center', paddingVertical: 12, backgroundColor: '#26282B', borderTopLeftRadius: 15, borderTopRightRadius: 15, minHeight: 46, shadowColor: '#000', shadowOffset: { width: 0, height: -2 }, shadowOpacity: 0.1, shadowRadius: 8, elevation: 5 },
